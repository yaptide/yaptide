...
def extract_shieldhit_from_tar_gz(
    archive_path: Path, unpacking_directory: Path, member_name: str, destination_dir: Path
):
...
def download_shieldhit_from_s3(
    ...
    download_and_decrypt_status = download_file(
        key=key,
        bucket=bucket,
        s3_client=s3_client,
        decrypt=decrypt,
            ...
            for tag in tags["TagSet"]:
                if tag["Key"] == "version" and tag["Value"] == version:
                    ...
                    s3_client.download_fileobj(
                        Bucket=bucket, Key=key, Fileobj=topas_temp_file, ExtraArgs={"VersionId": version_id}
                    )
                ...
                for tag in tags["TagSet"]:
                    if tag["Key"] == "topas_versions":
                        ...
                        if version in topas_versions:
                            ...
                            s3_client.download_fileobj(
                                Bucket=geant4_bucket, Key=key, Fileobj=temp_file, ExtraArgs={"VersionId": version_id}
                            )
...
def download_fluka_from_s3(
    download_dir: Path, endpoint: str, access_key: str, secret_key: str, bucket: str, password: str, salt: str, key: str
) -> bool:
    ...
    with tempfile.TemporaryDirectory() as tmpdir_name:
        ...
        if not download_and_decrypt_status:
            return False
        download_and_decrypt_status = extract_fluka_from_tar_gz(
            archive_path=tmp_archive, unpacking_directory=tmp_dir_path, destination_dir=download_dir
        )
