...
# Values arrive from the simulators as JSON numbers, so a page which happens to hold
# whole numbers is parsed into Python ints. Every array is therefore built with an
# explicit dtype instead of letting numpy infer one - an inferred int array would
# truncate on accumulation.
