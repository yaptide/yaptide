...
SHIELDHIT_CMD="sbatch --array=1-{n_tasks} {cmd_options} --parsable $ARRAY_SCRIPT > $OUT"
eval $SHIELDHIT_CMD
...
if [ -n "$JOB_ID" ] ; then
    COLLECT_CMD="sbatch --dependency=afterany:$JOB_ID {cmd_options} --parsable $COLLECT_SCRIPT > $OUT"
    eval $COLLECT_CMD
