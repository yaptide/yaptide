...
def test_run_simulation_with_flask(
    celery_app,
    celery_worker,
    client: Flask,
    ...
    for i in range(max_number_of_attempts):
        ...
        if jobs_data["job_state"] == "RUNNING":
            ...
            assert any(
                task["task_state"] in {"RUNNING", "PENDING", "COMPLETED"} for task in jobs_data["job_tasks_status"]
            )
        ...
        if jobs_data["job_state"] == "COMPLETED":
            for task in jobs_data["job_tasks_status"]:
                ...
                assert task["requested_primaries"] == requested_primaries, (
                    f"Requested primaries should be equal to {requested_primaries}"
                )
                assert task["simulated_primaries"] == requested_primaries, (
                    f"Simulated primaries should be equal to {requested_primaries}"
                )
    ...
    resp = client.get(
        "/results", query_string={"job_id": job_id, "estimator_name": estimator_name, "page_numbers": "0-1,3"}
    )
