...
def test_run_simulation_with_flask(celery_app,
    ...
    while True:
        ...
        if data["job_state"] == 'RUNNING':
            ...
            assert resp.status_code == 200  # skipcq: BAN-B101
            simulations = json.loads(resp.data.decode())
            assert {'message', 'simulations_count', 'simulations', 'page_count'} == set(simulations.keys())
            assert len(simulations["simulations"]) == 1
            start_time = simulations["simulations"][0]["start_time"]
            logging.info("start_time: %s", start_time)
            assert start_time is not None
            end_time = simulations["simulations"][0]["end_time"]
