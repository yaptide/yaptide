...
@pytest.mark.parametrize(
    "json_fixture", ["project_json_path", "shieldhit_payload_files_dict_path", "payload_editor_dict_path"]
)
...
def test_if_parsing_works_for_payload(payload_editor_dict_data: dict):
    ...
    files_dict = convert_editor_dict_to_files_dict(
        editor_dict=payload_editor_dict_data["input_json"], parser_type="shieldhit"
    )
    assert files_dict is not None
    validate_config_dict(files_dict)
...
def test_setting_primaries_per_task_for_editor(payload_editor_dict_data: dict):
    ...
    files_dict = convert_editor_dict_to_files_dict(
        editor_dict=json_project_data_with_adjust_prim_no, parser_type="shieldhit"
    )
