...
def test_watcher_falls_back_to_rest_until_the_aggregator_job_starts(monkeypatch, tmp_path):
    ...
    update = {"sim_id": 1, "task_id": 2, "update_key": "key", "backend_url": "http://backend", "update_dict": {}}

    # the aggregator job is still queued, its auth file does not exist yet
    assert watcher.send_task_update(**update)
    assert len(posted) == 1
