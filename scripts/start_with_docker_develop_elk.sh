...
docker compose --file docker-compose.yml --file docker-compose-develop.yml --file docker-compose.elk.yml --file docker-compose.fast.yml up  --detach

# if the fast version failed, try the slow version
if [ $? -ne 0 ]; then
  echo "Fast version failed, trying slow version:"
  docker compose --file docker-compose.yml --file docker-compose-develop.yml --file docker-compose.elk.yml up --detach
fi
