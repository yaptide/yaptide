...
func serializeBodies(w io.Writer, bodies []Body) {
	...
	writeBodyArgumentsLine := func(arguments []float64, argOffset int) {
		for i := 0; i < argumentsNumberInLine; i++ {
			if argOffset+i < len(arguments) {
				fmt.Fprintf(w, format.FloatToFixedWidthString(arguments[argOffset+i], 10))
			} else {
				// padding
				fmt.Fprintf(w, "%10s", "")
			}
...
func serializeZones(w io.Writer, zones []Zone) {
...
func serializeZoneToMaterialPairs(w io.Writer, zoneToMaterialPairs []ZoneToMaterial) {
	const mappingNumberPerLine = 14

	type IDGetter = func(ZoneToMaterial) int64

	for _, idGetter := range []IDGetter{
		func(ztm ZoneToMaterial) int64 { return int64(ztm.ZoneID) },
		func(ztm ZoneToMaterial) int64 { return int64(ztm.MaterialID) },
	} {
		for i := 0; i < len(zoneToMaterialPairs); i += 14 {
			var currentSlice []ZoneToMaterial
